...
class CSDAP_Dataset(BaseDataset):
    ...
    def __init__(self,
        ...
        if self.dem:
            print('FATAL: No DEM images available in CSDAP dataset yet.')
            exit()
        if self.slope:
            print('FATAL: No SLOPE images available in CSDAP dataset yet.')
            exit()

        super(CSDAP_Dataset, self).__init__(dset_name,
                                            root_dir,
                                            split,
                                            slice_params,
    ...
    def _split_data(self, region_dirs):
        ...
        for region_name, region_dir in region_dirs.items():
            ...
            assert len(
                region_image_paths
            ) > 0, f"No images found in {region_dir+'/images/*.tif'}"
    ...
    def _prepare_data(self):
        ...
        region_names = [r_dir.split('/')[-2] for r_dir in region_dirs]
        ...
        for image_path, region_name in image_paths:
            ...
            # Get label path
            label_path = os.path.join(self.root_dir, region_name,
                                      label_folder_name,
            ...
            n_images += 1
        print(
            f'Number of images in {self.dset_name} {self.split} dataset: {n_images}'
        )
    ...
    def _load_crop_norm_PS_image(self,
        ...
        # Normalize from uint16 to [0,1].
        image = np.clip(image / 2**14, 0, 1)
    ...
    def _to_RGB_S2(self, image):

        if self.channels == "RGB":
            if image.shape[0] != 3:
                raise ValueError(
                    f"Image should contain 3 bands if it is using RGB channels."
                )
            red_band = image[2]
            green_band = image[1]
            blue_band = image[0]
            ...
            if image.shape[0] != 4:
                raise ValueError(
                    f"Image should contain 4 bands if it is using RGB_NIR channels."
                )
            red_band = image[2]
            green_band = image[1]
            blue_band = image[0]
            ...
            if image.shape[0] != 10:
                raise ValueError(
                    f"Image should contain 10 bands if it is using ALL channels."
                )
            ...
            raise ValueError(
                f'Cannot handle channels variable of "{self.channels}".')

        # Adjust colors.
        red_band = red_band**(0.8)
        green_band = green_band**(0.8)
        blue_band = blue_band**(0.8)

        rgb_image = np.stack([red_band, green_band, blue_band])
        # print(f'RGB image: Min: {rgb_image.min()} | Mean: {rgb_image.mean()} | Max: {rgb_image.max()}')

        return rgb_image.transpose((1, 2, 0))

    def _to_RGB_PS(self, image):
        # Get RGB bands.
        if self.channels == 'RGB':
            red_band = image[0]
            green_band = image[1]
        ...
        # Adjust gamma values.
        red_band = red_band**(0.4)
        green_band = green_band**(0.4)
        blue_band = blue_band**(0.4)
...
if __name__ == "__main__":
    ...
    from PL_Support_Codes.tools import create_gif
    ...
    parser.add_argument('--eval_region', type=str, default=None, help='USA')
    ...
    # Load dataset object.
    dataset = CSDAP_Dataset(root_dir,
                            args.split,
                            slice_params,
                            sensor=args.sensor,
    ...
    for index in tqdm(args.ex_indices):
        ...
        create_gif([rgb_image, rgb_overlay],
                   f"./csdap_{dataset.sensor}_{index}.gif")
